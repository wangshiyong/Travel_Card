...
@interface WSYStatisticsViewController ()<WSYButtonDelegete>{
    PYOption *option;
}
...
@implementation WSYStatisticsViewController
...
- (void)viewDidLoad
{
    ...
    self.startTimeStr = [NSMutableString stringWithString:[dateFormatter stringFromDate:currentDate]];
    self.endTimeStr = [NSMutableString stringWithString:[dateFormatter stringFromDate:currentDate]];
...
{
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(showNewData)];
...
//    NSDictionary *params = @{@"TravelGencyID": [WSYUserDataTool getUserData:kTravelGencyID]};
    NSDictionary *param = @{@"StartTime": [WSYNSDateHelper getUTCFormateLocalDate:self.startTimeStr],
                            ...
                            @"TravelAgencyID": [WSYUserDataTool getUserData:kTravelGencyID]};
