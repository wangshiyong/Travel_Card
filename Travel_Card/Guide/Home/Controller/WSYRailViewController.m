...
@interface WSYRailViewController ()<MAMapViewDelegate> {
    NSTimer *timer;
    NSTimer *countTimer;
    int timeCount;
}
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = WSY(@"Electronic Fence");
    ...
    @weakify(self);
    NSDictionary *param = @{@"ElectronicFenceID": _ids};
    [[WSYNetworkTool sharedManager]post:WSY_GET_FENCE_COORDINATE params:param success:^(id responseBody) {
        @strongify(self)
        NSArray *array = [WSYRailModel mj_objectArrayWithKeyValuesArray:responseBody[@"Data"]];
        if (array.count == 0) {
            
        }else{
            _array = [NSMutableArray array];
            for(int i = 0; i < array.count; i++){
                WSYRailModel *model = array[i];
//                if ([[WSYUserDataTool getUserData:kForeign]isEqualToString:@"Foreign"]) {
//                    [_array addObject:[NSValue valueWithMKCoordinate:CLLocationCoordinate2DMake([model.Lat doubleValue], [model.Lng doubleValue])]];
//                } else {
//                    [_array addObject:[NSValue valueWithMKCoordinate:AMapCoordinateConvert(CLLocationCoordinate2DMake([model.Lat doubleValue], [model.Lng doubleValue]), AMapCoordinateTypeBaidu)]];
//                }
                [_array addObject:[NSValue valueWithMKCoordinate:AMapCoordinateConvert(CLLocationCoordinate2DMake([model.Lat doubleValue], [model.Lng doubleValue]), AMapCoordinateTypeBaidu)]];
            }
            CLLocationCoordinate2D coors[_array.count];
            NSInteger i = 0;
            ...
            //画围栏
            _polygon = [MAPolygon polygonWithCoordinates:coors count:_array.count];
            [self.mapView addOverlay:self.polygon];
            [_mapView setCenterCoordinate:[_array.lastObject MKCoordinateValue]];
        }
        [self hideHUD];
...
-(void)timerFresh
{
    ...
    [self.animations removeAllObjects];
    [self getTeamLocation];
    [self.mapView setCenterCoordinate:[self.array.lastObject MKCoordinateValue]];
}
...
- (void)timeSelected
{
    ...
    [alert addButton:@"30S" actionBlock:^(void) {
        ...
        [self timeRefresh];
        [self.mapView deselectAnnotation:self.customAnnotation animated:YES];
    }];
...
- (void)reportLocationServicesAuthorizationStatus:(CLAuthorizationStatus)status
{
    if(status == kCLAuthorizationStatusDenied)
    {
        ...
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:WSY(@"OK") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
...
//                    self.customAnnotation.onlineTime = [WSYNSDateHelper transformation:model.onLineTime];
//                    self.customAnnotation.gpsTime = [WSYNSDateHelper transformation:model.gpsTime];
...
- (MAOverlayRenderer *)mapView:(MAMapView *)mapView rendererForOverlay:(id <MAOverlay>)overlay
{
    if ([overlay isKindOfClass:[MAPolygon class]])
    {
        ...
        polygonRenderer.strokeColor = kThemeRedColor;
